...
def run_param(algorithm, experiment, method, method_file, n, d, k, l, a, b, min_deviation, termination_rounds, cl, std, dims_pr_cl,
    ...
    run_file = get_run_file(experiment, method_file, n, d, k, l, a, b, min_deviation, termination_rounds, cl, std,
                            dims_pr_cl, generator, round)
    ...
    if not os.path.exists(run_file):
        ...
        t0 = time.time()
        if method == "GPU-FAST-PROCLUS multi1" or method == "GPU-FAST-PROCLUS multi2" or method == "GPU-FAST-PROCLUS multi3" or method == "FAST-PROCLUS multi1":
...
def run_diff_n_param():
    ...
    to_plt = []
    for algo, algo_name, algo_file in [(PROCLUS, "PROCLUS", "PROCLUS"), (FAST_star_PROCLUS, "FAST*-PROCLUS", "PROCLUS-KEEP"),
                            (FAST_PROCLUS, "FAST-PROCLUS", "PROCLUS-SAVE"), (FAST_PROCLUS_multi, "FAST-PROCLUS multi1", "PROCLUS-PARAM"),
                            (GPU_PROCLUS, "GPU-PROCLUS", "GPU-PROCLUS"), (GPU_FAST_star_PROCLUS, "GPU-FAST*-PROCLUS", "GPU-PROCLUS-KEEP"),
                            (GPU_FAST_PROCLUS, "GPU-FAST-PROCLUS", "GPU-PROCLUS-SAVE"), (GPU_FAST_PROCLUS_multi, "GPU-FAST-PROCLUS multi1", "GPU-PROCLUS-PARAM"),
                            (GPU_FAST_PROCLUS_multi_2, "GPU-FAST-PROCLUS multi2", "GPU-PROCLUS-PARAM2"), (GPU_FAST_PROCLUS_multi_3, "GPU-FAST-PROCLUS multi3", "GPU-PROCLUS-PARAM3")]:
        avg_running_times = []
        ...
        for n in reversed(ns):
            ...
            for round in range(rounds):
                print("round:", round)
                running_time = run_param(algo, "inc_n_param", algo_name, algo_file, n, d, k, l, a, b, min_deviation,
...
def run_diff_d_param():
    ...
    for algo, algo_name, algo_file in [(PROCLUS, "PROCLUS", "PROCLUS"), (FAST_star_PROCLUS, "FAST*-PROCLUS", "PROCLUS-KEEP"),
        ...
        for d in ds:
            ...
            for round in range(rounds):
                print("round:", round)
                running_time = run_param(algo, "inc_d_param", algo_name, algo_file, n, d, k, l, a, b, min_deviation,
                                         termination_rounds, cl, std, dims_pr_cl, round)
                avg_running_time += running_time
...
def run_diff_n_param_large():
    n, d, k, l, a, b, min_deviation, termination_rounds, cl, std, dims_pr_cl, rounds = get_standard_params()
    ns = [1024000, 2048000, 4096000, 8192000] #[2000, 4000, 8000, 16000, 32000, 64000, 128000, 256000, 512000, 1024000, 2048000, 4096000, 8192000]
    ...
    to_plt = []
    for algo, algo_name, algo_file in [#(GPU_FAST_star_PROCLUS, "GPU-FAST*-PROCLUS", "GPU-FAST*-PROCLUS"), #(GPU_PROCLUS, "GPU-PROCLUS", "GPU-PROCLUS"),
                            (GPU_FAST_PROCLUS, "GPU-FAST-PROCLUS", "GPU-FAST-PROCLUS"), (GPU_FAST_PROCLUS_multi, "GPU-FAST-PROCLUS multi", "GPU-FAST-PROCLUS multi")]:
        avg_running_times = []
        for n in ns:
            # cl = max(1, n//4000)
            print("n:", n, "cl:", cl)
            ...
            for round in range(rounds):
                print("round:", round)
                running_time = run_param(algo, "inc_n_param_large", algo_name, algo_file, n, d, k, l, a, b, min_deviation,
                                         termination_rounds, cl, std, dims_pr_cl, round)
                avg_running_time += running_time
                running_times.append(running_time)
